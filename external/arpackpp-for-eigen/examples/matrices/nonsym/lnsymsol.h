...

{
  ...
  std::cout << std::endl
            << std::endl
            << "Testing ARPACK++ class ARluNonSymStdEig \n";
...
void Solution(MATRA &A, MATRB &B, ARluNonSymGenEig<ARFLOAT> &Prob, double lam)
/*
...

{
  ...
  if (Prob.EigenvaluesFound())
  {
    ...
    //====================saving to file======================
    std::cout << "Writing EVs to file." << std::endl;
    std::cout << std::endl;

    std::string str = std::to_string(int(lam));

    std::ofstream myfile;
    myfile.open("ev-val" + str+".dat");

    for (size_t i = 0; i < nconv; i++)
    {
      myfile << std::setprecision(17) << std::scientific << Prob.EigenvalueReal(i)<< "\t" << Prob.EigenvalueImag(i)<< "\n"; 
    }
    // for (size_t i = 0; i < u.rows(); i++)
    // {
    //     myfile << std::setprecision(17) << std::scientific << u(i) << "\n";
    // }
