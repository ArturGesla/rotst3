...
void calculateJacobianAndRhs(SparseMatrix<double> &jac, VectorXd &rhs, const VectorXd &u, int n, int neq, double Ro, double zmax);
void newtonIteration(const SparseMatrix<double> &jac, const VectorXd &rhs, VectorXd &u);
void saveResult(VectorXd &u);
void checkJacobian();

void rs1D(int n, double Ro, double zmax)
{
    {
        std::cout << "Rotst3 | 1D | based on Lingowood 1996" << std::endl;
